...
.PHONY: run stop restart logs install venv clean check-env setup help test test-gpt test-bot test-coverage test-api docker-up docker-down docker-logs docker-db deploy safe-run start stop-all install-full reinstall check-docker
...
# Проверка Docker (опционально, если хочешь автозапуск)
check-docker:
...
# Активация venv в отдельной шелл-сессии
venv:
	@if [ -d ".venv" ]; then \
...
help:
	...
	@echo "  make docker-logs  - Логи БД"
	@echo "  make docker-db    - Зайти в psql внутри контейнера"
	@echo ""
	@echo "🔧 Утилиты:"
	@echo "  make logs         - Просмотр локального лог-файла"
